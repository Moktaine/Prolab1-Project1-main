...
//Check if variables are already printed so they don't get printed several times
...
bool CanDrawSquare(int x, int y, int edgeLength, int polygonIndex){
    //Checks if any unit square inside given square is invalid
...
void RemoveSquareFromSquareArray(int x, int y, int edgeLength, int polygonIndex){
    //Removes every unit square inside given square
    for(int i = 0; i < edgeLength; i++){
        for(int j = 0; j < edgeLength; j++){
            int removeX = x + (j*10);
            int removeY = y + (i*10);
...
void DrawBigSquare(int x, int y, int edgeLength, int polygonIndex,bool isNeighbour){
    ...
    if(CanDrawSquare(x,y,edgeLength/10,polygonIndex)){
        ...
        RemoveSquareFromSquareArray(x, y /*- (window_HEIGHT/2)*/, edgeLength/10, polygonIndex);
    }
}
...
void CheckBigSquares(int squaresPerEdge){
    ...
    struct Coordinate TempSquaresInsidePolygon[3][600];
    struct Coordinate DrawableSquares[200];
    int squareCount;
...
void setCoordinates(char *buffer){
    ...
    //Split the vertices to polygons
...
int main(int argc, char **argv)
{
    ...
    //CREATE WINDOW AND DISPLAY THE SCENE
    //{
    ...
    WindowID1 = glutCreateWindow("Window1");
    ...
    WindowID2 = glutCreateWindow("Window2");
